...
class MergeJoin(BaseTool):
    ...
    def build_config(self, parent, params, on_change, columns=None):
        ...
        self.add_combobox(
            parent, "Join type", "how", ["inner", "left", "right", "outer"], params, on_change, 0
        )
        ...
        help_lbl = ttk.Label(
            parent,
            text=help_text.get(params.get("how", "inner"), ""),
            foreground="#7878a0",
            font=("Segoe UI", 7),
        )
        ...
        def rebuild() -> None:
            ...
            for i, pair in enumerate(key_pairs):
                ...
                def _lk(*_, p=pair, v=lk_var):
                    p["left_key"] = v.get()
                    on_change()

                lk_var.trace_add("write", _lk)
                ttk.Combobox(rf, textvariable=lk_var, values=left_cols, width=13).pack(
                    side="left", padx=(0, 4)
                )
                ...
                def _rk(*_, p=pair, v=rk_var):
                    p["right_key"] = v.get()
                    on_change()

                rk_var.trace_add("write", _rk)
                ttk.Combobox(rf, textvariable=rk_var, values=right_cols, width=13).pack(
                    side="left", padx=(0, 2)
                )
    ...
    def execute(self, params, inputs, log):
        ...
        left_only = (
            merged[merged["_merge"] == "left_only"].drop(columns=["_merge"]).reset_index(drop=True)
        )
        right_only = (
            merged[merged["_merge"] == "right_only"].drop(columns=["_merge"]).reset_index(drop=True)
        )
    ...
    def to_code(self, params, input_vars, output_var, connected_outs=None):
        ...
        if {"left_unmatched", "right_unmatched"} & set(outs):
            lines.append(
                f"_m = pd.merge({lv}, {rv}, left_on={lk!r}, right_on={rk!r}, how='outer', indicator=True)"
            )
            if "joined" in outs:
                incl = {
                    "inner": ["both"],
                ...
                if incl is None:
                    lines.append(
                        f"{output_var}_joined = _m.drop(columns=['_merge']).reset_index(drop=True)"
                    )
                else:
                    lines.append(
                        f"{output_var}_joined = _m[_m['_merge'].isin({incl!r})].drop(columns=['_merge']).reset_index(drop=True)"
                    )
            if "left_unmatched" in outs:
                lines.append(
                    f"{output_var}_left_unmatched = _m[_m['_merge']=='left_only'].drop(columns=['_merge']).reset_index(drop=True)"
                )
            if "right_unmatched" in outs:
                lines.append(
                    f"{output_var}_right_unmatched = _m[_m['_merge']=='right_only'].drop(columns=['_merge']).reset_index(drop=True)"
                )
        else:
            lines.append(
                f"{output_var}_joined = pd.merge({lv}, {rv}, left_on={lk!r}, right_on={rk!r}, how='{how}').reset_index(drop=True)"
            )
        return lines
...
class Union(BaseTool):
    ...
    def build_config(self, parent, params, on_change, columns=None):
        self.add_combobox(
            parent, "Match by", "match_by", ["name", "position"], params, on_change, 0
        )
...
class UniqueDuplicate(BaseTool):
    ...
    def build_config(self, parent, params, on_change, columns=None):
        ...
        self.add_column_multiselect(
            parent, "Key columns", "key_columns", params, on_change, 0, cols
        )
