...
def code(tool_type: str, params: dict, input_vars: list, output_var: str = "df_out",
         connected_outs=None) -> list[str]:
    tool = get_tool(tool_type)
...
def test_sort_ascending(simple_df):
    result = exe("sort", {"rules": [{"column": "salary", "order": "ascending"}]},
                 {"data": simple_df})
...
def test_sort_descending(simple_df):
    result = exe("sort", {"rules": [{"column": "salary", "order": "descending"}]},
                 {"data": simple_df})
    salaries = list(result["data"]["salary"])
...
def test_rename_columns(simple_df):
    result = exe("rename_columns",
                 {"renames": [{"from": "name", "to": "employee"}]},
                 {"data": simple_df})
...
def test_filter_codegen_no_conditions():
    ...
    assert any("pd.Series(True" in l for l in lines)
    full = "\n".join(lines)
    assert "df_in.index" in full   # the bug fix: {iv} must be interpolated


def test_export_codegen_import_csv():
    lines = code("import_csv",
                 {"file_path": "/tmp/x.csv", "delimiter": "comma"},
                 [], "df_out")
    assert len(lines) == 1
...
def test_sort_codegen():
    lines = code("sort",
                 {"rules": [{"column": "age", "order": "descending"}]},
                 ["df_in"], "df_out")
